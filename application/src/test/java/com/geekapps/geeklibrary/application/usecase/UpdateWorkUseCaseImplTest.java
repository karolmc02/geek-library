...
import com.geekapps.geeklibrary.application.usecase.work.UpdateWorkUseCaseImpl;
...
class UpdateWorkUseCaseImplTest {
        ...
        void setUp() {
                ...
                this.existingWork = new Work(this.workId, WorkType.MANGA, "Old Title",
                                "Old Description", this.oldAuthor, this.oldAuthor);

                final var newAuthorCommand = new PersonTO(newAuthorId, "New", "Author");
                final var newIllustratorCommand =
                                new PersonTO(newIllustratorId, "New", "Illustrator");

                this.command = new UpdateWorkCommand(this.workId, WorkType.LIGHT_NOVEL,
                                "Updated Title", "Updated Description", newAuthorCommand,
                                newIllustratorCommand);
        ...
        void shouldUpdateWorkSuccessfullyWhenWorkExists() {
                // Given
                Mockito.when(this.workRepository.findById(this.workId))
                                .thenReturn(this.existingWork);
        ...
        void shouldReturnNullWhenWorkDoesNotExist() {
                ...
                Mockito.verify(this.workRepository).findById(this.workId);
                Mockito.verify(this.workRepository, Mockito.never())
                                .save(ArgumentMatchers.any(Work.class));
        }
        ...
        void shouldUpdateWorkWithNullIllustrator() {
                ...
                final var commandWithoutIllustrator = new UpdateWorkCommand(this.workId,
                                WorkType.MANGA, "Title", "Description", authorCommand, null);

                final var author = new Person(authorId, "Author", "Name");
                final var updatedWorkWithoutIllustrator = new Work(this.workId, WorkType.MANGA,
                                "Title", "Description", author, null);
        ...
        void shouldCreateNewPersonIfNotFoundById() {
                ...
                final var commandWithNewPerson = new UpdateWorkCommand(this.workId, WorkType.MANGA,
                                "Title", "Description", personCommand, null);

                final var newPerson = new Person(newPersonId, "New", "Person");
                final var updatedWorkWithNewPerson = new Work(this.workId, WorkType.MANGA, "Title",
                                "Description", newPerson, null);
