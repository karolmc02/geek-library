...
class UpdateVolumeUseCaseImplTest {
  ...
  void setUp() {
    ...
    this.command = new UpdateVolumeCommand(this.workId, this.editionId, this.volumeId,
        "Volume 1 Updated", 1, priceTO, LocalDate.of(2024, 6, 15), "978-1-5555-6666-7", 250);
  ...
  void shouldUpdateVolumeSuccessfully() {
    ...
    Assertions.assertThat(result.getPrice().amount()).isEqualByComparingTo(new BigDecimal("14.99"));
  ...
  void shouldFailWhenVolumeDoesNotExist() {
    ...
    Mockito.doNothing().when(this.editionValidator).validateEditionExists(this.editionId,
        this.workId);
    Mockito.doThrow(new EntityNotFoundException("Volume", this.volumeId)).when(this.volumeValidator)
  ...
  void shouldCreateVolumeWithCorrectConstructorParameters() {
    ...
    Assertions.assertThat(capturedVolume.getPublicationDate()).isEqualTo(LocalDate.of(2024, 6, 15));
